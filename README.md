...
1. Download the code/scripts:
   ...
   ```
   This step will copy all the required files into your local directory.
2. Change the SLURM setting to reflect your HPC settings in your local copy of 
   ```RNA-SeqEZPZ/scripts/slurm_config_var.sh```

3. Go to the ```RNA-SeqEZPZ``` directory and download the singularity image:
   ```
...
```
   ...
   ![run_analysis_screenshot](run_analysis_screenshot.png)
...
2. In order to run the test dataset, first you will need to select project folder. 
   ...
   ![run_example_0](run_example_0.png)
...
3. Next, you will need to fill out the form.
   ...
   | ----------------| ----------------------------------------------------------------------------------------------------------|
   | iEF714R1        | cells with knockdown of endogenous EWSR1::FLI1 followed by rescue with a EWSR1::FLI1 construct replicate 1|
   | iEF714R2        | cells with knockdown of endogenous EWSR1::FLI1 followed by rescue with a EWSR1::FLI1 construct replicate 2|
   | iEF563R1        | cells with knockdown of endogenous EWSR1::FLI1 followed by rescue with a EWSR1::ETV4 construct replicate 1|
   | iEF563R2        | cells with knockdown of endogenous EWSR1::FLI1 followed by rescue with a EWSR1::ETV4 construct replicate 2|
   | iEF197R1        | cells with knockdown of endogenous EWSR1::FLI1 followed by rescue with an empty vector replicate 1        |
   | iEF197R2        | cells with knockdown of endogenous EWSR1::FLI1 followed by rescue with an empty vector replicate 2        |
   
   The goal of the analysis is to find genes regulated by EWSR1::FLI1 and genes regulated by EWSR1::ETV4.
   Therefore, we are going to compare iEF714 which we will be our iEF_EF group with iEF197 as control.
   We will also compare iEF563 which will be our iEF_EE4 group with iEF197 as control.
   ...
   * 2 rows for iEF_empty group since we have two replicates with NA as control
   ...
   ![run_example_1](run_example_1.png)
...
6. After clicking on ```Run full analysis```, you can click on ```Log``` then click on ```Refresh list``` to see the content of ```run_rnaseq_full.out```
  ...
  ![run_example_2](run_example_2.png)
...
7. When the entire pipeline is done, you can scroll down on ```run_rnaseq_full.out``` and see similar message as pictured below:
   ![run_example_3](run_example_3.png)
...
8. Once full analysis is finished, you can click on ```QCs``` tab to see the Quality Control metrics generated.
   ![run_example_4](run_example_4.png)
...
10. In the ```Plots``` tab, inserting another comparison group will show the overlap between the two groups of comparisons.
   ...
   ![run_example_6](run_example_6.png)
...
![volcano_example](volcano_example.png)
...
![overlap_example](overlap_example.png)
...
![upset_example](upset_example.png)
...
![pathway_example](pathway_example.png)
