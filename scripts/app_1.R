...
#### run analysis tab ######
 ...
 react.setup.grp.ctrl.name <- reactive({
  grp.ctrl.name <- sapply(grep("setup\\.grp.+\\.ctrl\\.name", x = names(input), value = TRUE),
                           function(x) input[[x]])
 ...
 react.setup.grp.rep.name <- reactive({
  grp.rep.name <- sapply(grep("setup\\.grp.+\\.rep\\.name", x = names(input), value = TRUE),
                          function(x) input[[x]])
 ...
 # shinyFileChoose(input, 'setup_grp1_r1_files', root=volumes, 
 #                                filetypes=c('', 'gz'))
 # # file.lst as reactive to store previous choices
 # setup.grp.r1.file.lst <- reactiveValues(datapath="")
 # #react.setup.grp1.r1.file <- reactive(input$setup_grp1_r1_files)
 # react.setup.grp.r1.files <- reactive({
 #  grp.r1.files <- sapply(grep("setup_grp.+\\_r1_files", x = names(input), value = TRUE),
 #                     function(x) input[[x]])
 #  print("grp.r1.files")
 #  print(grp.r1.files)
 #  grp.r1.files
